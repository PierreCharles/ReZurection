...
Phaser.Plugin.VirtualJoystick.prototype.init = function (x, y, baseDiameter, stickDiameter, limit, baseColor, stickColor) {
    ...
    this.nub.input.enableDrag();
    this.nub.events.onInputDown.add(this.startDrag, this);
    this.nub.events.onInputUp.add(this.stopDrag, this);
    this.nub.events.onDragUpdate.add(this.move, this);
};

Phaser.Plugin.VirtualJoystick.prototype.setLocation = function (x, y) {
    this.nub.position.set(x, y);
    this.base.position.set(x, y);
    this.location.set(x, y);
    this.baseCircle = new Phaser.Circle(x, y, this.baseCircle.diameter);
}
...
Phaser.Plugin.VirtualJoystick.prototype.move = function (nub, pointer) {
...
Phaser.Plugin.VirtualJoystick.prototype.setVelocity = function (sprite, minSpeed, maxSpeed) {
    ...
    if (this.force === 0 && minSpeed === 0) {
        ...
        var speed = (maxSpeed - minSpeed) * this.force + minSpeed;
...
Phaser.Plugin.VirtualJoystick.prototype.getOrientation = function () {

    if (this.distance == 0)
        return null;
    else
        return new Phaser.Point(this.deltaX, this.deltaY);
};

Phaser.Plugin.VirtualJoystick.prototype.getForce = function() {
    return this.force;
}

Phaser.Plugin.VirtualJoystick.prototype.destroy = function () {
